...
(define s "515	912	619	2043	96	93	2242	1385	2110	860	2255	621	1480	118	1230	99
...
(define lines
  (map nums (string-split s "\n")))

(define (biggest-diff nums)
  (- (apply max nums) (apply min nums)))

(define (checksum lines)
  (apply + (map biggest-diff lines)))

(define (divisible-pair nums)
  (define (evenly-divides x items)
    (match items
      [(cons item rest)
       (if (zero? (modulo item x))
           (list x item)
           (evenly-divides x rest))]
      [_ null]))
  (match (sort nums <)
    [(cons item rest)
     (match (evenly-divides item rest)
       [(list x y) (list x y)]
       [_ (divisible-pair rest)])]
    [_ null]))

(define (divisible-pair-divided nums)
  (match (divisible-pair nums)
    [(list x y) (/ y x)]
    [_ null]))

(define (sum-divisions lines)
  (apply + (map divisible-pair-divided lines)))
